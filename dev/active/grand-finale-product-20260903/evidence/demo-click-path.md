...
1. **Dashboard** `/` — Say: six official G2B notices, separated by evidence level. Point at the two KPI tiles. The second tile reads `Open deadlines 1 · 5 closed · next 2026.09.03 · 16:00 KST` until 16:00 KST today, and `Open deadlines 0 · All deadlines passed · historical public sources` afterwards. Every row carries an `Open` or `Closed` tag next to a KST deadline. The blue `Open verified PURSUE replay →` button sits beside the filters, above the table.
