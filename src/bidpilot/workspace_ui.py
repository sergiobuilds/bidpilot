...
def koat_dashboard(rows: Sequence[Mapping[str, object]]) -> str:
    ...
    recent = "".join(
        ...
        f'<span class="rtype">{esc(str(row.get("status") or ""))}</span></div>'
        f'<div class="rdetail">{esc(row.get("title"))}</div></div>'
        f'<span class="rts">{esc(_catalog_date(row.get("retrieved_at")))}</span></div>'
        for row in rows[:5]
    )
    body_rows = "".join(
        "<tr>"
    ...
    return (
        ...
        f'<span class="filter-chip">Source found {len(found)}</span></div></header>'
        '<section class="kpi-band">'
        f'<div class="kpi"><i class="tick"></i><span class="kpi-lab">Public sources</span>'
        f'<div class="kpi-val"><strong class="kpi-num">{len(rows)}</strong><span class="kpi-unit">notices</span></div>'
        '<p class="kpi-ctx">Official G2B URLs</p></div>'
        ...
        '<p class="kpi-ctx">Supplier evidence required</p></div>'
        ...
        + "".join(
            f'<div class="frow"><div class="fstep"><span class="nm">{label}</span>'
            f'<span class="rt">{note}</span></div><div class="ftrack"><i class="fbar" style="width:{width}%"></i>'
            f'<strong class="fcount{(" outside" if width == 0 else "")}" style="left:{("14px" if width else "12px")}">{count}</strong></div></div>'
            for label, note, width, count in (
                ("Discovered", "official URLs", 100, len(rows)),
                ("Source reviewed", "deep contract", 17, len(reviewed)),
                ("Decision", "REVIEW", 17, review_count),
                ("Strategy", "after PURSUE", 0, 0),
                ("Proposal", "after strategy", 0, 0),
                ("Owned work", "after review", 0, 0),
            )
        )
        + '</div></div><aside class="panel"><div class="shead"><h2 class="t-headline1">Recent activity</h2>'
        '<span class="meta">latest source records</span></div><div class="recent">'
        f"{recent}</div></aside></section>"
...
def koat_tender_detail(
    ...
    *,
    ...
    return (
        ...
        '<nav class="detail-nav"><a href="?">Dashboard</a><a class="current" href="#">Tender detail</a></nav>'
        ...
        f'<div class="an-foot"><span class="flagbadge">{"Supplier evidence required" if reviewed else "Source review required"}</span></div></div></section>'
